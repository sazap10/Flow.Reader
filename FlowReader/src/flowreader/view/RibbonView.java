...
public class RibbonView extends Group {
	...
	public void setCloudZoomLevels() {
		...
		while (pageCount >= 1) {
			...
			ArrayList<WordCloudView> currClouds = mergeClouds(
					cloudLevels.get(levelCount), levelCount);
	...
	// clouds
	// IMPORTANT: only works with an even number of clouds, final page is missed
	// out at this point,
	...
	public ArrayList<WordCloudView> mergeClouds(ArrayList<WordCloudView> input,
		...
		for (WordCloudView i : input) {
			if (haveb && (b != null)) {
				...
				WordCloudView newCloud = new WordCloudView(i, b, new Rectangle(
						x, y, pageWidth * scale, (pageHeight / 3) * scale));
